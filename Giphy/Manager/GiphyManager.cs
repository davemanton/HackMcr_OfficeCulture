...
using OfficeCulture.Giphy.Extensions;
using Newtonsoft.Json.Linq;

namespace OfficeCulture.Giphy.Manager
{
    public class GiphyManager
    {
        ...
        public async Task<Models.Giphy> GetGiphyTaskAsync(string query)
        {
            Models.Giphy objGiphy = null;
            HttpResponseMessage response = await _client.GetAsync(
                    $"{_giphyEndpoint}?q={query}&limit={_giphyLimit}&rating={_giphyRating}&api_key={_giphyKey}");
            if (response.IsSuccessStatusCode)
            {
                objGiphy = await response.Content.ReadAsJsonAsync<Models.Giphy>();
            }
            return objGiphy;
        }

        public async Task<Models.Giphy> RunAsync(string query)
        {
